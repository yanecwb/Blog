      ...
      <div class="usershow" ref="usershow">
        ...
        <div class="userlabel">
          <span class="userlabel_span1">
            ...
            <span>{{ formatConstellation(new Date(parseInt(userInfo.birthday)).toLocaleString().split(' ')[0]) || "星 座" }}</span>
          </span>
  ...
  methods: {
    ...
    formatConstellation(birthday){
      ...
      switch(getMonth){
        ...
        case 2:
          if(getDay<=18){res = '水瓶座';} else if(getDay>=19){res = '双鱼座'} break;
        case 3:
          if(getDay<=20){res = '双鱼座'} else if(getDay>=21){res = '白羊座'} break;
        case 4:
          if(getDay<=19){res = '白羊座'} else if(getDay>=20){res = '金牛座'} break;
        case 5:
          if(getDay<=20){res = '金牛座'} else if(getDay>=21){res = '双子座'} break;
        case 6:
          if(getDay<=21){res = '双子座'} else if(getDay>=22){res = '巨蟹座'} break;
        case 7:
          if(getDay<=22){res = '巨蟹座'} else if(getDay>=23){res = '狮子座'} break;
        case 8:
          if(getDay<=22){res = '狮子座'} else if(getDay>=23){res = '处女座'} break;
        case 9:
          if(getDay<=22){res = '处女座'} else if(getDay>=23){res = '天枰座'} break;
        case 10:
          if(getDay<=23){res = '天枰座'} else if(getDay>=24){res = '天蝎座'} break;
        case 11:
          if(getDay<=22){res = '天蝎座'} else if(getDay>=23){res = '射手座'} break;
        case 12:
    ...
    async changeInfoHandler(){
      ...
      if(changeInfo.nickname && changeInfo.nickname.length>5){
        ...
        this.miniMessage('昵称不能超过五位字符','warning')
        return
      }
      if(changeInfo.sex && (changeInfo.sex != 'man' && changeInfo.sex != 'woman')){
        this.inspectObj = 'sex'
        this.miniMessage('请填写正确的性别格式man或者woman','warning')
        return
      }
      if(changeInfo.birthday && (!/^((([0-9]{3}[1-9]|[0-9]{2}[1-9][0-9]{1}|[0-9]{1}[1-9][0-9]{2}|[1-9][0-9]{3})-(((0[13578]|1[02])-(0[1-9]|[12][0-9]|3[01]))|((0[469]|11)-(0[1-9]|[12][0-9]|30))|(02-(0[1-9]|[1][0-9]|2[0-8]))))|((([0-9]{2})(0[48]|[2468][048]|[13579][26])|((0[48]|[2468][048]|[3579][26])00))-02-29))$/.test(changeInfo.birthday))){
        ...
        this.miniMessage('请填写正确的日期格式，例：1999-01-01','warning')
        return
      }
